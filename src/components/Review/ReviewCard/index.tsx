...
export const ReviewCard = ({ review }: ReviewCardProps) => {
  ...
  return (
    <div className="relative">
      {isThumbNailShow ? (
        <div className="absolute w-screen h-real-screen bg-dark">모달</div>
      ) : null}
      <div className="py-5 pl-[17px] pr-5 bg-grayscale-20 text-grayscale-80 rounded-[5px]">
        <div className="flex justify-between mb-[17px]">
          <div className="flex items-center space-x-[10px]">
            ...
            />
            <div className="headline2-bold">{review.nickName}</div>
          </div>
          <div
          ...
          >
            <Like />
            <span className="body2-medium">{review.likeCount}</span>
          </div>
        </div>
        ...
        <div className="flex space-x-[14px] max-h-[90px] mb-[19px]">
          <div
            className="relative"
            onClick={() => setIsThumbNailShow((prev) => !prev)}
          >
            <img
        ...
        <div className="flex space-x-2 overflow-x-auto scrollbar-hide mb-[19px]">
          {review.locationReview.map((lReview, index) => (
            <ReviewTag key={index}>{lReview}</ReviewTag>
          ))}
        </div>

        <div className="flex justify-between caption-12">
          <div className="text-grayscale-50">{review.date}</div>
          <div className="flex items-center space-x-2 text-grayscale-60">
            <div onClick={() => gotoReviewDetail(review.reviewId)}>더 보기</div>
            <BackArrow
              width="5"
              height="11"
              viewBox="0 0 11 20"
